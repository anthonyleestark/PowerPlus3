...
BOOL CGridCell::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT id, UINT nChar)
{
    if ( m_bEditing )
	{      
        if (m_pEditWnd)
	...
	{   
		...
		m_pEditWnd = new CInPlaceEdit(pGrid, rect, style, id, nRow, nCol, GetText(), nChar);
    }
    return TRUE;
}
